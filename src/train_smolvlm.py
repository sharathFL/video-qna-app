...
class LiveDashboardCallback(TrainerCallback):
    ...
    def __init__(self, output_dir, processor, dataset, update_every_n=PLOT_UPDATE_EVERY_N_STEPS, sample_indices=None, pred_every_n_frame=None, training_output_dir=None, model=None, resuming=False):
    ...
    def _save_input_grid_and_run_inference(self, step):
        ...
        if not HAS_MATPLOTLIB:
            return
        # Rotate which 5 samples we show: same spread (0%, 25%, 50%, 75%, 100%) but offset by step so each update shows different frames
